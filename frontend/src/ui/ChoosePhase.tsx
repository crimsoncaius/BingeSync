...
export function ChoosePhase({
  ...
  const usedPlaceIdSet = useMemo(
    () => new Set(usedGooglePlaceIds),
    [usedGooglePlaceIds],
  );
    ...
    <div className="grow p-4 md:p-8 lg:p-12 max-w-5xl mx-auto w-full pb-32 md:pb-12">
      <section className="mb-12">
        <header className="mb-8">
          ...
          {session.title ? (
            <p className="text-primary font-headline font-semibold text-lg mb-2">
              {session.title}
            </p>
          ) : null}
          <p className="text-secondary max-w-lg">
            Add your favorite spots to the pool. We&apos;ll vote on them in the
            next phase.
          </p>
        </header>
            ...
            <form className="relative mb-6" onSubmit={onAddSubmit}>
              <div className="flex gap-2">
                        ...
                        {suggestions.map((s, index) => {
                          ...
                          return (
                            <button
                              ...
                              key={
                                s.placeId ? s.placeId : `q-${s.name}-${index}`
                              }
                              onMouseDown={(e) => {
                                e.preventDefault();
                                if (!busy && !alreadyInPool)
                                  onSuggestionPick(s);
                              }}
                              type="button"
                            >
                ...
                <button
                  ...
                  disabled={
                    busy || optionInput.trim().length < 2 || atPickLimit
                  }
                  type="submit"
                >
                  {renderActionLabel(
              ...
              {atPickLimit ? (
                <p className="text-xs text-secondary mt-2">
                  You&apos;ve reached the maximum picks for this room (
                  {pickLimit} per person).
                </p>
              ) : null}
            </form>
          </div>
            ...
            <div className="bg-surface-container-lowest rounded-xl p-6 shadow-xs grow border-t-4 border-secondary">
              ...
              <div className="space-y-3" ref={optionListRef}>
                {session.options.length === 0 ? (
                  <p className="text-sm text-on-surface-variant italic">
                    No options yet. Add at least one place.
                  </p>
                ) : (
              ...
              <div className="mt-6 flex items-start gap-3 p-3 bg-secondary-container/30 rounded-lg">
                ...
                <p className="text-[11px] leading-relaxed text-on-secondary-container font-medium">
                  Only you can see these until everyone&apos;s done. Your
                  choices are private for now.
                </p>
              </div>
            </div>
      ...
      <div className="mt-12 relative h-48 rounded-2xl overflow-hidden bg-surface-container-highest">
        ...
        <div className="absolute bottom-6 left-6">
          ...
          <p className="text-sm text-secondary">
            {participantCount}{" "}
            {participantCount === 1 ? "person is" : "people are"} in this room.
          </p>
        </div>
      </div>
